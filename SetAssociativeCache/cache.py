...
class Cache(ABC):
    ...
    def __init__(self, set_count, set_size):
        ...
        :type set_size: int
        """
        self.contents = [collections.OrderedDict() for _ in range(set_count)]
    ...
    def put(self, key, value):
        ...
        if not (isinstance(key, self.key_type) and isinstance(value, self.value_type)):
            raise TypeError(
                "Either key or value " "does not match the " "existing key value types"
            )
        ...
        if len(self.contents[set_]) >= self.set_size:
            raise ReplacementStrategyException(
